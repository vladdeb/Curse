...
initialization
  ...
  AddAud(BSUIR[2][1], TAuditory.Create(  40, 435, 101, 'à', 3));
  ...
  AddAud(BSUIR[2][1], TAuditory.Create( 367, 435, 107, 'à', 3));
  ...
  begin
    if i = 4 then
      continue;
    for var j := 0 to High(BSUIR[2][i]) do
    begin
      for var k := j + 1 to High(BSUIR[2][i]) do
      begin
        distx := Abs(BSUIR[2][i][j].Pos.X - BSUIR[2][i][k].Pos.X);
        disty := Abs(BSUIR[2][i][j].Pos.Y - BSUIR[2][i][k].Pos.Y);
        if ((distx <= 20) or (disty <= 20)) and (distx <= 300) and (disty <= 250) then
        begin
          AddEdge(BSUIR[2], BSUIRGraph[2], i*100 + j, i*100 + k);
        end;
      end;
    end;
  end;
  AssignFile(graphFile, 'graph3.dat');
  rewrite(graphFile);
  for var i := 1 to 1100 do
  begin
    Vertex.size := 0;
    for var j := Low(BSUIRGraph[2][i]) to High(BSUIRGraph[2][i]) do
    begin
      Vertex.Vert[Vertex.size] := BSUIRGraph[2][i][j];
      Inc(Vertex.size);
    end;
